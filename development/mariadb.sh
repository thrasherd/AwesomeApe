...
install_mariadb()
{
    ...
    aptitude -q=2 update >> ~/install.log 2> ~/error.log
    aptitude -q=2 -y build-dep mysql-server >> ~/install.log 2> ~/error.log
    ...
    scripts/mysql_install_db >> ~/install.log 2> ~/error.log
    /usr/local/mysql/bin/mysqld_safe & >> ~/install.log 2> ~/error.log
    echo "done."
}
